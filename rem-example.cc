...
{
    std::string remMode = "BeamShape"; // BeamShape, CoverageArea, UeCoverage
    std::string simTag = "";
    ...
    // propgation loss model
    enum BandwidthPartInfo::Scenario scenarioEnum = BandwidthPartInfo::InH_OfficeMixed;
    ...
    double frequency = 4.139e9; // in Hz
    double bandwidth = 100e6; // in Hz
    uint16_t numerology = 1;
    // double txPower = 0.1995; // in watts
    double txPower = 0.1; // in watts

    // Antenna Parameters
    double hBS; // Depend on the scenario (no input parameters), base station's heights
    double hUT; // UE's height
    bool isoUe = false;
    ...
    {
        std::cout << "Enable InH-OfficeMixed!" << std::endl;
        hBS = 3;
        hUT = 1.5;
    ...
    if (enableBuildings)
    {
        std::cout << "Enable Buildings!" << std::endl;
    ...
    {
        std::cout << "enable dir-omni beamforming!" << std::endl;
        idealBeamformingHelper->SetAttribute(
            "BeamformingMethod",
